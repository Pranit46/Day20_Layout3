...
let users = fetch("https://api.jikan.moe/v3/search/anime?q=dragonballz&page=1")
            .then((res) => res.json())
            .then((result)=> {
                users = result;
                console.log(users); 
        ...
        for (i = j; i < k; i++) {
                    ...
                    <img src="${users.results[i].image_url}" class="avtar" alt="Avatar" style="width:100%">
                    <div class="container">
                         <h4><b>${users.results[i].title}</b></h4> 
                         <p>${users.results[i].synopsis}</p> 
                     </div>
                </div>`
            }
}